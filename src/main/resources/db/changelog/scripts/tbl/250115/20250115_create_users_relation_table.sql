...
-- changeset kwang:20250115_create_users_relation_table.sql
...
alter table tbl_users
    add constraint fk_users_content foreign key (logo_file_id) references tbl_contents (id);

update tbl_users set nickname = 'admin', phone_number = '01012341234' where login_id = 'admin';
alter table tbl_users modify column nickname varchar(255) not null comment '닉네임';
alter table tbl_users modify column phone_number varchar(100) not null comment '핸드폰번호';

create table if not exists tbl_users_sns
(
    id                 bigint primary key not null auto_increment comment '인덱스id',
    users_id           bigint             not null comment '사용자인덱스id',
...
alter table tbl_users_sns add constraint fk_users_sns_users foreign key (users_id) references tbl_users(id);